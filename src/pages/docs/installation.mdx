...
> Compatibility (v0.2.0): deploy Issuer, WebUI, and use `openauthster-shared` v0.2.x together. Mixing 0.1.x with 0.2.x APIs is not supported.
...
> Note: make sure to use the version matching your issuer/WebUI (e.g. `0.2.x` for the latest release).
...
1. Update client apps to `openauthster-shared@0.2.x`.
2. Redeploy the issuer with the v0.2.0 code (no database migration is required).
3. Redeploy the WebUI with v0.2.0 to match the new admin/session endpoints.
4. Verify health/version endpoints: `/health`, `/version`.
